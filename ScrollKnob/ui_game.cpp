...
static void redraw(void) {
  ...
  bool flash = (s_flashUntil != 0);
  lv_obj_set_style_text_color(s_num, lv_color_hex(flash ? COL_GREEN : COL_TX), 0);
...
void game_encoder(int rawDir) {
  ...
  if (s_pos == s_secret[s_tumbler] && dirCorrect(mv)) startHold();
  redraw();
}
...
void game_tick(void) {
  ...
  if (s_flashUntil && now >= s_flashUntil) {
    s_flashUntil = 0;
    if (!s_open) lv_obj_set_style_text_color(s_num, lv_color_hex(COL_TX), 0);
  }
  if (s_holdActive) {
